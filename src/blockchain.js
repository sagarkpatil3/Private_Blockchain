...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            let sign = `${address}:${new Date().getTime().toString().slice(0,-3)}:starRegistry`;
            resolve(sign)
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(currentTime < (timeFromMessage +(5*60*1000))){
                ...
                reject('New star can be added in 5 Minutes');
            }
        });
    }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
           ...
           if(block !== undefined){
               ...
               reject('No Block found by this hash')
           }
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            self.chain.map((block)=>{
                ...
                if(block.height > 0){
                    ...
                    if(blockHash !== previousBlockHash ){
                        errorLog.push(`Error: At Block height :${block.height} Doesn't match previousBlock hash Value`);
                    }
                }
                index++;
            });
            ...
            Promise.all(Promises).then((results) =>{
                index = 0;
                results.map(validBlock =>{
                    if(!validBlock){
                        errorLog.push(`Error - At Block height: ${self.chain[index].height} - data is tampered.`)
                    }
                    index++;
                })
